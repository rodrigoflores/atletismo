class Teste < ActiveRecord::Base
  ...
  def resultado
    tmp = ["","",""]
    tmp[0] << "#{self.minutos}' " if self.minutos && self.minutos != 0
    tmp[0] << "#{self.segundos}'' "  if self.segundos && self.segundos != 0
    tmp[0] << "#{self.decimos} "  if self.decimos && self.decimos != 0
    tmp[1] << "#{self.kg} kg " if self.kg && self.kg != 0
    tmp[2] << "#{self.metros} m " if self.metros && self.metros != 0
    tmp[2] << "#{self.centimetros} cm " if self.centimetros && self.centimetros != 0
    tmp.delete("")
    return tmp.join " | "
  end
